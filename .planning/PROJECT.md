...
**Goal:** 消除 TypeScript 类型隐患，统一事件层，添加 ErrorBoundary 和 operation tracing，让代码库达到生产级可维护性标准。

**Target features:**
- **A — TypeScript 类型安全**：消除 `gateway-bridge.ts` 中的 `any`，IPC handler 参数与返回值全类型化
- **B — EventBus 统一事件监听**：合并 chatStore / openclawStore 事件注册为统一 EventBus + React Context 封装
- **C — 错误处理与监控**：React ErrorBoundary + operation tracing（request ID 贯穿 RPC 调用链）
- **D — Workspace 统一**：FileExplorer 使用 OpenClaw workspace 路径，移除独立 workspace 逻辑

**Target features — 已验证（from v1.4）：**
- macOS 公证（Phase 9/19）配置就绪，真实提交待用户提供 Apple ID
...
- v1.5 milestone **planned**：Phase 20–23, P2 架构与质量债
...
*Last updated: 2026-04-06 — v1.5 P2 架构与质量债 planned (Phase 20–23)*
