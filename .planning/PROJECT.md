...
- ✓ **v1.6 P3 UI 完善冲刺** — SkillsPanel 安装进度 + IM 频道管理 UI 精简
- ✓ **v1.7 Backlog 清理 + 性能优化冲刺** — Gateway 状态统一管理（gatewayStore + useGatewayStatus）+ 四个面板空状态补全（Avatar/TaskBoard/SkillsMarket/Mcp）+ UX 补全（Avatar Header/快捷键/Toast/ClawHub CLI）+ 性能优化（启动计时/IPC 去重+缓存/React selector/virtualization/内存泄漏修复）
...
## Current Milestone: v1.8 规划中

**Goal:** 规划下一个里程碑

**Status:** 📋 Planning — 准备收集需求

**Out of scope:** macOS 公证（P0 — 待用户提供 Apple ID）
...
- v1.6 milestone **complete**：Phases 24–25, 7/7 requirements (P3 UI 完善冲刺)
- v1.7 milestone **complete**：Phases 26–30, 20/20 requirements (Backlog 清理 + 性能优化)
- v1.8 **planned**：待规划
...
| Zustand per-field selectors | 避免 destructuring 整个 store 导致的不必要重渲染 | ✓ 正确（v1.7） |
| React.lazy + Suspense 非首屏组件 | 减少首屏 bundle 大小，提升 TTI | ✓ 正确（v1.7） |
| IPC 请求去重（500ms TTL）+ 热点方法缓存 | 减少冗余 RPC 调用，提升响应速度 | ✓ 正确（v1.7） |
| CSS contain + 选择性 animation | 替代 react-window，在不引入依赖的情况下优化消息列表滚动 | ✓ 正确（v1.7） |
| gatewayStore disconnect 清理 | 防止订阅泄漏导致内存增长 | ✓ 正确（v1.7） |
...
*Last updated: 2026-04-08 — v1.7 milestone completed, v1.8 planning*
