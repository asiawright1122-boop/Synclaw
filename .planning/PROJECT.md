...
## Current Milestone: v1.7 Backlog 清理 + 性能优化

**Goal:** 扫清 P1/P2 backlog 积压，提升应用响应速度和用户体验

**Target features:**

**Backlog 清理：**
- Onboarding API key 验证（ping Gateway 确认连接）
- Gateway 断连 UI（专属断连 banner + 重连按钮）
- 关键面板 Empty states 补全
- Avatar 当前状态显示（ChatView Header）
- WEB_API_BASE 降级（API 不可用时优雅降级）
- Keyboard shortcuts 扩展（Cmd+, 等 macOS 标准快捷键）
- Toast 体系完整性（关键操作全覆盖）
- ClawHub CLI 存在性检测

**性能优化：**
- 启动速度优化（Electron 启动链路分析）
- IPC 延迟优化（批量请求、缓存）
- React 重渲染优化（memo/selector 审查）
- 内存占用分析（排除内存泄漏）
...
- v1.7 milestone **planned**：Phases 26+, Backlog 清理 + 性能优化
...
*Last updated: 2026-04-07 — v1.7 Backlog 清理 + 性能优化 planned*
