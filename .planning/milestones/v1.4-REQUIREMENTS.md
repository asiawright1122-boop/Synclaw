# Requirements Archive: v1.4 安全加固冲刺
...
- [x] **SHELL-01**: `shell:openExternal` 阻止 `javascript:` 协议（防止 URL 栏 XSS）
- [x] **SHELL-02**: `shell:openExternal` 阻止 `data:` 协议（防止 data: URL 滥用）
- [x] **SHELL-03**: `shell:openExternal` 仅允许 `https:` 和 `mailto:` 协议，其他全部拒绝
- [x] **SHELL-04**: 白名单拦截后显示 toast 提示「链接被安全策略拦截」

## SBX — Sandbox 对接

- [x] **SBX-01**: Gateway 启动时配置 `sandbox: { mode: "non-main" }`（exec 工具在子进程运行）
- [x] **SBX-02**: 配置 `sandbox.docker.network: "none"`（沙箱内禁用网络）
- [x] **SBX-03**: 配置 `sandbox.docker.readOnlyRoot: true`（根目录只读）
- [x] **SBX-04**: SecurityPanel 添加 Sandbox 开关（默认开启），UI 层控制是否应用 sandbox config
